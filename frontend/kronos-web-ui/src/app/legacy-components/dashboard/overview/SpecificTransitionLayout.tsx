...
const SpecificTransitionLayout: React.FC<SpecificTransitionLayoutProps> = ({ jobId, selectedActivityPair }) => {
  ...
  const barChartAvgDataByResource = useFetchData(
    `/activity_transitions_avg_by_resource/${jobId}/${sourceActivity}/${destinationActivity}`,
  );
  const barChartTotalDataByResource = useFetchData(
    `/activity_transitions_by_resource/${jobId}/${sourceActivity}/${destinationActivity}`,
  );
  ...
  const processingTimePercentage = +parseFloat(
    ((generalOverviewData.processing_time / totalCycleTime) * 100).toFixed(1),
  );
  ...
  const highestSourceText = isMaxPairDefined
    ? `Handover: ${overviewData.max_wttotal_pair[0]} - ${overviewData.max_wttotal_pair[1]}\n${dhmToString(
        secondsToDhm(overviewData.max_wttotal_pair[2]),
      )}`
    : "No highest handover";
  const highestAvgSourceText = isMaxPairDefined
    ? `Handover: ${overviewData.max_wttotal_avg_pair[0]} - ${overviewData.max_wttotal_avg_pair[1]}\n${dhmToString(
        secondsToDhm(overviewData.max_wttotal_avg_pair[2]),
      )}`
    : "No highest average handover";

  const filteredData = cteTableData.data.filter(
    (item: { source_activity: string; target_activity: string }) =>
      item.source_activity === sourceActivity && item.target_activity === destinationActivity,
  );

  const transformedCteImpact = filteredData.map(
    (item: {
      cte_impact: {
        batching_impact: any;
        contention_impact: any;
        extraneous_impact: any;
        prioritization_impact: any;
        unavailability_impact: any;
      };
    }) => ({
      ...
      >
        <Grid item xs={4}>
          ...
          >
            ...
            <div>
              {overviewData.specific_case_count} / {overviewData.total_case_count}
            </div>
        ...
        <Grid item xs={4}>
          ...
          >
            ...
            >
              ...
              <div style={{ position: "absolute", right: 0, top: "50%", transform: "translateY(-50%)" }}>
                <FormControl variant="outlined" size="small" style={{ width: "120px" }}>
                  ...
                  <Select value={selectedMode} onChange={handleChange} label="Data Mode">
            ...
            <div>{wtValue === 0 ? "0" : dhmToString(secondsToDhm(wtValue))}</div>
        ...
        <Grid item xs={4}>
          ...
          >
            ...
            <div style={{ textAlign: "left", marginBottom: "35px", whiteSpace: "pre-line" }}>
              {highestAvgSourceText}
            </div>
        ...
        <Grid item xs={12}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <Tooltip
              title={<span style={{ fontSize: "1rem" }}>Waiting time of transition by the causes of waiting.</span>}
            >
        ...
        <Grid item xs={12}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
              ...
              <Tooltip
                title={
                  <span style={{ fontSize: "1rem" }}>
                    Waiting time between a pair of resources executing activities of the selected transition,
                    categorized by the causes of waiting.
                  </span>
                }
              >
                <IconButton size="small" aria-label="info about waiting time causes">
            ...
            <FormControl variant="outlined">
              <InputLabel>Data Mode</InputLabel>
              <Select value={selectedMode2} onChange={(e) => setSelectedMode2(e.target.value)} label="Data Mode">
        ...
        <Grid item xs={4}>
          <div style={{ display: "flex", alignItems: "center", marginBottom: "8px", justifyContent: "center" }}>
            ...
            <Tooltip
              title={
                <span style={{ fontSize: "1rem" }}>
                  Measures the amount of value-added time in a process. Calculated as the ratio of processing time to
                  cycle time. The closer CTE is to 100%, the relatively less waiting time is in the process.
                </span>
              }
            >
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", alignItems: "center", marginBottom: "8px" }}>
            ...
            <Tooltip
              title={
                <span style={{ fontSize: "1rem" }}>
                  The potential value of CTE if the waiting time due to each cause is eliminated. Helps identify which
                  causes of waiting time to focus on to achieve greater improvements.
                </span>
              }
            >
