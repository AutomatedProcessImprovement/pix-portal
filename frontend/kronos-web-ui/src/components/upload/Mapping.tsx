...
export default function MappingDialog(props: SimpleDialogProps) {
  ...
  return (
    <Dialog onClose={() => handleClose(true)} open={open}>
      ...
      <DialogContent>
        <DialogContentText>Here you can assign the right column names to each column for processing.</DialogContentText>
        <List>
          {selectedValue.map((columnName, index) => (
            <ListItem key={columnName}>
              ...
              <FormControl sx={{ m: 1, minWidth: 250 }}>
                ...
                <Select id={columnName} label="Type" defaultValue={""} onChange={(e) => handleChange(e, index)}>
                  <MenuItem value="">
                    <em>None</em>
                  </MenuItem>
