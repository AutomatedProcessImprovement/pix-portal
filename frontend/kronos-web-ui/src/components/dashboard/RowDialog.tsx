...
const transitions_columns: GridColDef[] = [
  ...
  },
  {
    ...
    flex: 0.01,
    type: "number",
    valueFormatter: (params) => moment.duration(params?.value, "seconds").format("d[D] HH[H] mm[M]"),
  },
  {
    field: "batching_wt",
    headerName: "Batching",
    ...
    return (
      <Modal onClose={handleClose} open={open}>
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            Potential CTE if waiting times of a particular cause are eliminated per resource pair in the selected
            transition
          </Typography>
          <>
            <DataGrid
