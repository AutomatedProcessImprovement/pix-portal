...
const COLORS = [
  _colorDict.batching,
  _colorDict.prioritization,
  _colorDict.contention,
  _colorDict.unavailability,
  _colorDict.extraneous,
];
...
function TransitionsBarChart({ data, selectedWTType }: Props) {
  ...
  const isSingleActivityType =
    !isActivityTransitionType && dataArray.length > 0 && dataArray[0].hasOwnProperty("activity");
  ...
  if (isActivityTransitionType) {
    for (const dataKey in dataArray) {
      let out = {
        name:
          dataArray[dataKey].source_activity.trim().charAt(0).toUpperCase() +
          dataArray[dataKey].source_activity.trim().slice(1).toLowerCase() +
          " - " +
          dataArray[dataKey].target_activity.trim().charAt(0).toUpperCase() +
          dataArray[dataKey].target_activity.trim().slice(1).toLowerCase(),
    ...
    for (const dataKey in dataArray) {
      let out = {
        name:
          dataArray[dataKey].activity.trim().charAt(0).toUpperCase() +
          dataArray[dataKey].activity.trim().slice(1).toLowerCase(),
        total_wt: dataArray[dataKey].total_wt,
        batching_wt: dataArray[dataKey].batching_wt,
        prioritization_wt: dataArray[dataKey].prioritization_wt,
        contention_wt: dataArray[dataKey].contention_wt,
    ...
    for (const dataKey in dataArray) {
      ...
      let formattedSource =
        typeof source === "string"
          ? source.trim().charAt(0).toUpperCase() + source.trim().slice(1).toLowerCase()
          : "Unknown";
      let formattedTarget =
