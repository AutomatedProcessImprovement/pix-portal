...
const Upload = () => {
    ...
    try {
      ...
      axios(config)
        .then((res: any) => {
          ...
          let counter = 0;
          let f = setInterval(() => {
            axios.get("http://193.40.11.233/jobs/" + job.id).then((r: any) => {
              let j = r.data;
  ...
  return (
    <>
        ...
        <Grid item xs={6}>
          <Paper elevation={5} sx={{ p: 3, minHeight: "20vw" }}>
            <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <Grid container>
                      ...
                      <Grid item xs={9}>
                        <Typography variant="body1" align="left">
                          <Link
                            href="https://owncloud.ut.ee/owncloud/s/rZ4dSoTzwpwfpci"
                            target="_blank"
              ...
              <Grid item xs={12}>
                <CustomDropzoneArea acceptedFiles={[".csv"]} setSelectedLogFile={setSelectedLogFile} />
              </Grid>
            </Grid>
          </Paper>
        </Grid>
        <MappingDialog open={open} onClose={handleClose} selectedValue={selectedValue} />
        <Grid item xs={12}>
          <LoadingButton disabled={!areFilesPresent()} variant="contained" onClick={onContinueClick} loading={loading}>
            Upload event log
          </LoadingButton>
        </Grid>
        <Grid item xs={12}>
      ...
      {snackMessage && (
        <CustomizedSnackbar message={snackMessage} severityLevel={snackColor} onSnackbarClose={onSnackbarClose} />
      )}
    </>
  );
};
