  ...
  async function onSubmit(data: ResetPasswordSchema) {
    try {
      ...
      if (ok) {
        ...
        methods.setError("root", { message: "Password reset failed" });
      }
    } catch (error) {
      console.error(error);
      methods.setError("root", { message: "An error occurred while resetting the password" });
    } finally {
      setIsLoading(false);
    }
  }
          ...
          <Form className="flex flex-col justify-center items-center h-full" onSubmit={methods.handleSubmit(onSubmit)}>
            ...
            <div className="flex justify-center mt-8">
              <button type="submit" className="w-44" disabled={isLoading}>
                {isLoading ? "Generating email..." : "Send Link"}
              </button>
            </div>
      ...
      {isEmailSent && (
        <div className="m-8 text-center">
          <p className="text-3xl">
            <span className="text-blue-600">Email has been sent!</span>
          </p>
          <p className="text-base mt-2">Please check your inbox and click on the link to reset your password.</p>
        </div>
      )}
    </div>
  );
