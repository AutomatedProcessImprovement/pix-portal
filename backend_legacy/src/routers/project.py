...
from src.models.models import File as F
from src.models.models import Project as P
from src.models.models import Tag as T
...
async def get_all_projects(
    authorization: Annotated[str | None, Header()], db: Session = Depends(get_db)
):
...
async def create_new_project(
    ...
    if not user:
        raise HTTPException(
            status_code=404, detail="User not found, are you logged in?"
        )
...
async def update_new_project(
    ...
    try:
        _project = (
            db.query(P)
            .filter(P.id == project_id, P.owner_id == user.id)
            .update({P.name: name})
        )
    except Exception:
...
async def get_project_files(
    ...
    res = (
        db.query(U)
        .join(P)
        .filter(P.owner_id == user.id)
        .join(F)
        .filter(F.project_id == project_id)
        .join(T)
        .filter(T.id == F.tag_id)
        .with_entities(F, T)
        .all()
    )
...
async def delete_project(
    ...
    if not project:
        raise HTTPException(
            status_code=404,
            detail="Project/user combination not found, are you the owner?",
        )
    name = project.name
    ...
    return {
        ...
        "message": f"Project {name} and related files successfully removed",
    }
