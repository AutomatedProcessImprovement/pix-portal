...
class AssetService(SelfAuthenticatingService):
    ...
    async def download_asset(self, asset_id: str, output_dir: Path, token: Optional[str] = None) -> Asset:
        asset = await self.get_asset(asset_id, token=token)
        asset.url = await self.get_asset_location(asset_id, token=token)
    ...
    async def get_asset_location(self, asset_id: str, token: Optional[str] = None) -> str:
